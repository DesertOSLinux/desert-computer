...
class MainWindow:
    ...
    def defineComponents(self):
        ...
        self.dialog_about.set_program_name(_("Desert Computer"))
    ...
    def showVolumeSizes(self, row_volume):
        ...
        if vl.get_mount() != None:
            ...
            #     f'<span size="small"><b>{int(file_info["free_kb"])/1000/1000:.2f} GB</b> {_("свободно из")} {int(file_info["total_kb"])/1000/1000:.2f} GB</span>')
            row_volume._lbl_volume_size_info.set_markup("<span size='small'><b>{:.2f} GB</b> {} {:.2f} GB</span>".format(
                int(file_info['free_kb'])/1000/1000, _("своболно из"),int(file_info["total_kb"])/1000/1000))
            ...
            try:
                if file_info["usage_percent"] >= 0.9:
                    row_volume._pb_volume_size.get_style_context().add_class("desert-computer-progress-90")
    ...
    def addVolumeRow(self, vl, listbox, is_removable):
        ...
        lbl_volume_name.set_markup("<b>{}</b><small> ( {} )</small>".format(
            vl.get_name(),_("Диск отключен, нажмите, чтобы включить")))
    ...
    def addDisksToGUI(self):
        ...
        try:
            if root_info["usage_percent"] >= 0.9:
                self.pb_root_usage.get_style_context().add_class("desert-computer-progress-90")
        except Exception as e:
            print("progress css exception: {}".format(e))
    ...
    def addHardDisksToList(self):
        ...
        for dr in drives:
            if dr.has_volumes() and not dr.is_removable():
                ...
                listbox.get_style_context().add_class("desert-computer-listbox")
                # frame.add(listbox)
    ...
    def on_btn_format_removable_clicked(self, btn):
        ...
        subprocess.Popen(["desert-usb-formatter", file_info["device"]])
    ...
    def on_menu_aboutdesert_clicked(self, button):
        self.popover_menu.popdown()
        try:
            subprocess.Popen(["desert-about"])
        except Exception as e:
            print("on_menu_aboutdesert_clicked Exception: {}".format(e))
    ...
    def notify(self, message_summary="", message_body="", icon="desert-computer"):
        try:
